...
// AuthenticatedOperation is the trusted application projection of a freshly
// verified follow-up authorization. It binds one mutation, not a reusable
// session. Human Coordination v1 does not expose an external Action ASB
// profile; structural validation does not establish provenance, so untrusted
// network, RPC, queue, or plugin input must not construct this value directly.
...
type TransitionRecord struct {
	...
	AcceptanceContextDigest string    `json:"acceptance_context_digest,omitempty"`
	EvidenceRef             string    `json:"evidence_ref,omitempty"`
...
// AcceptanceContextDigest binds application state that this package cannot
// model directly, such as a current TaskCoord Assignment projection.
...
type Transition struct {
	...
	// Auth carries the complete verifier-created acceptance projection to the
	// application Store for commit-time freshness checks. It is not part of the
	// durable Action document; Record retains the audit provenance and digest.
	Auth *AuthenticatedOperation `json:"-"`
}
