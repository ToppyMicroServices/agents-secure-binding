...
func (s Snapshot) Validate() error {
	...
	if s.LastTransition.Kind == EventAccept {
		...
		if s.Revision != 1 || !s.CreatedAt.Equal(s.UpdatedAt) || s.LeaseGeneration != 0 ||
			s.RecoveryAttempts != 0 || s.Checkpoint != nil || s.Reconciliation != nil {
			return invalidSnapshot("acceptance must be the initial snapshot")
		}
		digest, err := AcceptanceRequestDigest(Definition{
			EventID: s.LastTransition.EventID, ActionID: s.ActionID, ActionDigest: s.ActionDigest,
			OwnerID: s.OwnerID, RecoveryPolicy: s.RecoveryPolicy,
			AcceptanceContextDigest: s.LastTransition.AcceptanceContextDigest,
		})
		if err != nil || digest != s.LastTransition.MutationDigest {
			return invalidSnapshot("acceptance transition digest does not match snapshot")
		}
	} else {
		if !transitionAllowed(s.LastTransition.Kind, s.LastTransition.From, s.LastTransition.To) {
			return invalidSnapshot("last transition edge does not match event kind")
		}
		minimumRevision := minimumTransitionRevision(s.LastTransition.Kind, s.LastTransition.From)
		if minimumRevision == 0 || s.Revision < minimumRevision {
			return invalidSnapshot("last transition cannot occur at the recorded revision")
		}
		if !reasonAllowed(s.LastTransition.Kind, s.LastTransition.Reason.Code) {
...
func (r TransitionRecord) Validate() error {
	...
	if r.Kind == EventLeaseExpired {
		if r.ActorID != "" || r.AuthorizationID != "" || r.ProofID != "" || r.MutationDigest != "" ||
			r.AcceptanceContextDigest != "" {
			return fmt.Errorf("trusted transition must not contain authenticated provenance")
		}
	} else {
		for name, value := range provenance {
			if value == "" {
		...
		if err := validateDigest("last_transition.mutation_digest", r.MutationDigest); err != nil {
			return err
		}
		if r.Kind == EventAccept {
			if err := validateDigest("last_transition.acceptance_context_digest", r.AcceptanceContextDigest); err != nil {
				return err
			}
		} else if r.AcceptanceContextDigest != "" {
			return fmt.Errorf("acceptance_context_digest is only valid for ACCEPT")
		}
	}
	if r.EvidenceRef != "" {
...
func validateReason(r Reason) error {
	...
	if len(r.Detail) > maxDetailBytes || !safeText(r.Detail) {
...
func validateID(name, value string) error {
	if value == "" || strings.TrimSpace(value) != value || len(value) > maxIDBytes || !safeText(value) {
...
func validateReference(name, value string) error {
	if value == "" || strings.TrimSpace(value) != value || len(value) > maxReferenceBytes || !safeText(value) {
		return fmt.Errorf("%s is missing, unsafe, or too long", name)
	}
	return nil
}
