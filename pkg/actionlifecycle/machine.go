...
// NewSnapshot returns the first durable, authenticated ACCEPTED snapshot. Auth
// must be a projection created by an enclosing ASB verifier for this exact
// acceptance, and its MutationDigest must equal AcceptanceRequestDigest(def).
// The caller commits it with expected revision zero before acknowledging
// Action acceptance.
func NewSnapshot(def Definition) (Transition, error) {
	...
	if def.Auth == nil {
		return Transition{}, ErrAuthenticationRequired
	}
	if err := validateAuthenticatedAcceptance(def, *def.Auth); err != nil {
	...
	record := TransitionRecord{
		...
		Reason:                  reason,
		At:                      at,
		ActorID:                 def.Auth.ActorID,
		AuthorizationID:         def.Auth.AuthorizationID,
		ProofID:                 def.Auth.ProofID,
		MutationDigest:          def.Auth.MutationDigest,
		AcceptanceContextDigest: def.AcceptanceContextDigest,
	}
	snapshot := Snapshot{
		Schema:         SchemaV1,
	...
	auth := *def.Auth
	return Transition{Snapshot: snapshot, Record: record, Auth: &auth}, nil
}

func validateAuthenticatedAcceptance(def Definition, auth AuthenticatedOperation) error {
	...
	if auth.Operation != EventAccept || auth.ActionID != def.ActionID || auth.ActionDigest != def.ActionDigest {
		return fmt.Errorf("%w: authenticated acceptance binding mismatch", ErrInvalidEvent)
	}
	if err := validateDigest("auth.mutation_digest", auth.MutationDigest); err != nil {
		return fmt.Errorf("%w: %v", ErrInvalidEvent, err)
	}
	digest, err := AcceptanceRequestDigest(def)
	if err != nil {
		return err
	}
	if auth.MutationDigest != digest {
		return fmt.Errorf("%w: authenticated acceptance digest mismatch", ErrInvalidEvent)
	}
	if auth.IssuedAt.IsZero() || auth.ExpiresAt.IsZero() || auth.IssuedAt.After(def.AcceptedAt) || !auth.ExpiresAt.After(def.AcceptedAt) {
		return fmt.Errorf("%w: authenticated acceptance is not current", ErrInvalidEvent)
	}
	return nil
}
...
func Apply(current Snapshot, event Event) (Transition, error) {
	...
	case EventRenewLease:
		...
		if event.Auth == nil || event.Lease.ExpiresAt.After(event.Auth.ExpiresAt) {
			return Transition{}, fmt.Errorf("%w: renewed lease exceeds authenticated operation expiry", ErrInvalidEvent)
		}
		lease := *event.Lease
...
func validateAuthenticatedOperation(current Snapshot, event Event, auth AuthenticatedOperation) error {
	...
	if auth.MutationDigest != digest {
		return fmt.Errorf("%w: authenticated mutation digest mismatch", ErrInvalidEvent)
	}
...
func acquireLease(next *Snapshot, event Event, generation uint64) error {
	...
	if event.Lease.ExpiresAt.After(event.Auth.ExpiresAt) {
		return fmt.Errorf("%w: executor lease exceeds authenticated operation expiry", ErrInvalidEvent)
	}
	lease := *event.Lease
	next.ExecutorLease = &lease
...
func transitionRecord(current, next Snapshot, event Event) TransitionRecord {
	...
	if event.Auth != nil {
		...
		record.MutationDigest = event.Auth.MutationDigest
	}
	return record
}
...
func transitionAllowed(kind EventKind, from, to State) bool {
	switch kind {
	case EventStart:
		return from == StateAccepted && to == StateRunning
	case EventWait:
		return from == StateRunning && to == StateWaiting
	case EventPause:
		return (from == StateRunning || from == StateWaiting) && to == StatePaused
	case EventResume:
		return (from == StateWaiting || from == StatePaused) && to == StateRunning
	case EventRequestCancel:
		return (from == StateRunning && to == StateCanceling) ||
			((from == StateAccepted || from == StateWaiting || from == StatePaused) && to == StateCanceled) ||
			(from == StateOrphaned && to == StateIndeterminate)
	case EventComplete:
		return (from == StateRunning || from == StateCanceling) && to == StateSucceeded
	case EventFail:
		return (from == StateRunning || from == StateCanceling) && to == StateFailed
	case EventConfirmCanceled:
		return from == StateCanceling && to == StateCanceled
	case EventMarkIndeterminate:
		return (from == StateRunning || from == StateCanceling || from == StateOrphaned) && to == StateIndeterminate
	case EventLeaseExpired:
		return (from == StateRunning || from == StateCanceling) && to == StateOrphaned
	case EventTakeover:
		return from == StateOrphaned && to == StateRunning
	case EventBeginReconciliation:
		return (from == StateIndeterminate || from == StateOrphaned) && to == StateIndeterminate
	case EventResolveReconciliation:
		return from == StateIndeterminate &&
			(to == StatePaused || to == StateSucceeded || to == StateFailed || to == StateCanceled)
	case EventRenewLease:
		return (from == StateRunning && to == StateRunning) ||
			(from == StateCanceling && to == StateCanceling)
	default:
		return false
	}
}

// minimumTransitionRevision returns the earliest revision at which kind may
// follow from. It is a lower-bound consistency check for compact snapshots;
// the authoritative Store must still retain and validate the full history.
func minimumTransitionRevision(kind EventKind, from State) uint64 {
	if kind == EventResolveReconciliation {
		// Resolution requires a prior transition that began reconciliation.
		return 5
	}
	switch from {
	case StateAccepted:
	...
	case StateRunning:
		return 3
	case StateWaiting, StatePaused, StateOrphaned, StateCanceling, StateIndeterminate:
		return 4
	default:
		return 0
	}
