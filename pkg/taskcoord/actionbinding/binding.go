...
func validateTransition(transition actionlifecycle.Transition) error {
	...
	if last.EventID != record.EventID || last.Kind != record.Kind || last.From != record.From ||
		...
		last.ProofID != record.ProofID || last.MutationDigest != record.MutationDigest ||
		last.AcceptanceContextDigest != record.AcceptanceContextDigest ||
		last.EvidenceRef != record.EvidenceRef ||
