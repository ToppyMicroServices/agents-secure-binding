...
func TestSucceededActionOnlyMakesFulfillmentEligible(t *testing.T) {
	...
	bindActionEvent(t, &completeEvent)
...
func TestDependencyWaitCycleProjectsToExistingDeadlockDetector(t *testing.T) {
	...
	for index, id := range []string{"a", "b"} {
		...
		prepareDependencyWaitEvent(t, binding, running.Snapshot, dependencies, &waitEvent)
...
func TestNonDependencyWaitIsAnExternalEscape(t *testing.T) {
	...
	prepareDependencyWaitEvent(t, bindingA, runningA.Snapshot, dependencies, &waitEventA)
...
func TestDependencyResumeRequiresSameSatisfiedTopology(t *testing.T) {
	...
	evidence, err := DependencyResumeEvidence(binding, assignment, waiting.Snapshot, wait, dependencies)
	if err != nil {
		t.Fatal(err)
	}
...
func TestServiceRunsHumanTaskActionLifecycle(t *testing.T) {
	...
	dependencies[0].Satisfied = true
	evidence, err := DependencyResumeEvidence(view.Binding, view.Assignment, view.Action, wait, dependencies)
	if err != nil {
		t.Fatal(err)
	}
	resume.EvidenceRef = evidence
	bindActionEvent(t, &resume)
	clock = resumeAt
...
func TestMemoryStoreRejectsDependencyTOCTOUWithoutPartialCommit(t *testing.T) {
	...
	err = store.CommitDependencyWait(
		...
		dependencies, waitEvent, transition, view.Binding, wait,
	)
...
func TestMemoryStoreActionCASHasOneWinner(t *testing.T) {
	...
	events := make([]actionlifecycle.Event, 2)
	...
	clock = at.Add(5 * time.Second)
	var wg sync.WaitGroup
	results := make(chan error, len(transitions))
	for index, transition := range transitions {
		wg.Add(1)
		go func(event actionlifecycle.Event) {
			defer wg.Done()
			results <- store.CommitExecutionTransition(
				ctx, view.Assignment.Revision, view.Assignment, view.Action.Revision,
				view.Action, event, transition, view.Binding,
			)
		}(events[index])
	}
	wg.Wait()
...
func TestMemoryStoreRejectsFutureDatedTransitionAtCommit(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	at := time.Date(2026, 8, 14, 18, 30, 0, 0, time.UTC)
	clock := at.Add(2 * time.Second)
	assignment := acceptedAssignment(t, "assignment:future", "task:future", "agent:owner", at)
	store, err := NewMemoryStoreWithClock(
		[]taskcoord.Assignment{assignment}, nil, func() time.Time { return clock },
	)
	if err != nil {
	...
	view, err := acceptWithTestAuth(t, service, ctx, assignment, clock, AcceptRequest{
		AssignmentID: assignment.AssignmentID, EventID: "event:accept:future",
		ActionID: "action:future", ActionDigest: "sha256:" + strings.Repeat("e", 64),
		RecoveryPolicy: actionlifecycle.RecoveryPolicy{
			Mode: actionlifecycle.RecoveryRestartIdempotent, MaxAttempts: 1,
	...
	eventAt := at.Add(10 * time.Second)
	start := authenticatedActionEvent(view.Action, actionlifecycle.EventStart, eventAt, "executor:future")
	start.Reason = actionlifecycle.Reason{Code: actionlifecycle.ReasonStarted}
	start.Lease = lease(view.Action.LeaseGeneration+1, "executor:future", eventAt)
	start.Auth.IssuedAt = at.Add(time.Second)
	start.Auth.ExpiresAt = at.Add(30 * time.Minute)
	bindActionEvent(t, &start)
	...
	clock = at.Add(5 * time.Second)
	err = store.CommitExecutionTransition(
		ctx, view.Assignment.Revision, view.Assignment, view.Action.Revision,
		view.Action, start, transition, view.Binding,
	)
	if !errors.Is(err, actionlifecycle.ErrInvalidEvent) {
		t.Fatalf("future-dated commit error = %v", err)
	}
	stored, err := store.Load(ctx, view.Action.ActionID)
	if err != nil {
		t.Fatal(err)
	}
	if stored.State != actionlifecycle.StateAccepted || stored.Revision != view.Action.Revision {
		t.Fatalf("future-dated commit changed Action: %+v", stored)
	}
}

func TestMemoryStoreRejectsExecutionThroughOrdinaryCommit(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	at := time.Date(2026, 8, 14, 18, 45, 0, 0, time.UTC)
	clock := at.Add(2 * time.Second)
	assignment := acceptedAssignment(t, "assignment:path", "task:path", "agent:owner", at)
	store, err := NewMemoryStoreWithClock(
		[]taskcoord.Assignment{assignment}, nil, func() time.Time { return clock },
	)
	if err != nil {
	...
	view, err := acceptWithTestAuth(t, service, ctx, assignment, clock, AcceptRequest{
		AssignmentID: assignment.AssignmentID, EventID: "event:accept:path",
		ActionID: "action:path", ActionDigest: "sha256:" + strings.Repeat("e", 64),
		RecoveryPolicy: actionlifecycle.RecoveryPolicy{
			Mode: actionlifecycle.RecoveryRestartIdempotent, MaxAttempts: 1,
			IdempotencyKey: "idempotency:path",
		},
	})
	if err != nil {
		t.Fatal(err)
	}
	start := authenticatedActionEvent(view.Action, actionlifecycle.EventStart, at.Add(3*time.Second), "executor:path")
	start.Reason = actionlifecycle.Reason{Code: actionlifecycle.ReasonStarted}
	start.Lease = lease(view.Action.LeaseGeneration+1, "executor:path", start.At)
	bindActionEvent(t, &start)
	transition, err := actionlifecycle.Apply(view.Action, start)
	if err != nil {
		t.Fatal(err)
	...
	clock = start.At
	if err := store.CommitAuthorizedTransition(
		ctx, view.Action.Revision, view.Action, start, transition,
	); !errors.Is(err, ErrWrongCommitPath) {
		t.Fatalf("ordinary commit accepted execution transition: %v", err)
	}
}

func TestMemoryStoreRejectsMismatchedTransitionMutationDigest(t *testing.T) {
	t.Parallel()
	t.Run("execution", func(t *testing.T) {
		ctx := context.Background()
		at := time.Date(2026, 8, 14, 18, 50, 0, 0, time.UTC)
		clock := at.Add(2 * time.Second)
		assignment := acceptedAssignment(t, "assignment:digest:execution", "task:digest:execution", "agent:owner", at)
		store, err := NewMemoryStoreWithClock(
			[]taskcoord.Assignment{assignment}, nil, func() time.Time { return clock },
		)
		if err != nil {
		...
		view, err := acceptWithTestAuth(t, service, ctx, assignment, clock, AcceptRequest{
			AssignmentID: assignment.AssignmentID, EventID: "event:accept:digest:execution",
			ActionID: "action:digest:execution", ActionDigest: "sha256:" + strings.Repeat("e", 64),
			RecoveryPolicy: actionlifecycle.RecoveryPolicy{
				Mode: actionlifecycle.RecoveryRestartIdempotent, MaxAttempts: 1,
				IdempotencyKey: "idempotency:digest:execution",
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		start := authenticatedActionEvent(view.Action, actionlifecycle.EventStart, at.Add(3*time.Second), "executor:digest")
		start.Reason = actionlifecycle.Reason{Code: actionlifecycle.ReasonStarted}
		start.Lease = lease(view.Action.LeaseGeneration+1, "executor:digest", start.At)
		bindActionEvent(t, &start)
		transition, err := actionlifecycle.Apply(view.Action, start)
		if err != nil {
			t.Fatal(err)
		}
		transition.Record.MutationDigest = "sha256:" + strings.Repeat("0", 64)
		clock = start.At
		err = store.CommitExecutionTransition(
			ctx, view.Assignment.Revision, view.Assignment, view.Action.Revision,
			view.Action, start, transition, view.Binding,
		...
		if !errors.Is(err, ErrInvalidBinding) {
			t.Fatalf("mismatched execution transition error = %v", err)
		}
	})

	t.Run("ordinary", func(t *testing.T) {
		at := time.Date(2026, 8, 14, 18, 55, 0, 0, time.UTC)
		store, _, view, clock := newRunningActionService(t, at)
		complete := executorEvent(view.Action, actionlifecycle.EventComplete, at.Add(4*time.Second))
		complete.Reason = actionlifecycle.Reason{Code: actionlifecycle.ReasonCompleted}
		complete.ResultRef = "urn:result:digest-mismatch"
		bindActionEvent(t, &complete)
		transition, err := actionlifecycle.Apply(view.Action, complete)
		if err != nil {
			t.Fatal(err)
		}
		transition.Record.MutationDigest = "sha256:" + strings.Repeat("0", 64)
		*clock = complete.At
		err = store.CommitAuthorizedTransition(
			context.Background(), view.Action.Revision, view.Action, complete, transition,
		)
		if !errors.Is(err, ErrInvalidBinding) {
			t.Fatalf("mismatched ordinary transition error = %v", err)
		}
	})
}

func TestServiceRejectsExecutionWhenAssignmentChangesBeforeCommit(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	at := time.Date(2026, 8, 14, 19, 0, 0, 0, time.UTC)
	clock := at.Add(2 * time.Second)
	assignment := acceptedAssignment(t, "assignment:execution-race", "task:execution-race", "agent:owner", at)
	released, err := taskcoord.Apply(
		assignment,
		taskEvent(assignment, taskcoord.OperationRelease, at.Add(4*time.Second)),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	store := &assignmentRaceStore{MemoryStore: inner, replacement: released.Assignment}
	service, err := NewService(store, func() time.Time { return clock })
	if err != nil {
		t.Fatal(err)
	}
	view, err := acceptWithTestAuth(t, service, ctx, assignment, clock, AcceptRequest{
		AssignmentID: assignment.AssignmentID, EventID: "event:accept:execution-race",
		ActionID: "action:execution-race", ActionDigest: "sha256:" + strings.Repeat("e", 64),
		RecoveryPolicy: actionlifecycle.RecoveryPolicy{
			Mode: actionlifecycle.RecoveryRestartIdempotent, MaxAttempts: 1,
			IdempotencyKey: "idempotency:execution-race",
		},
	})
	if err != nil {
		t.Fatal(err)
	}
	start := authenticatedActionEvent(view.Action, actionlifecycle.EventStart, at.Add(3*time.Second), "executor:race")
	start.Reason = actionlifecycle.Reason{Code: actionlifecycle.ReasonStarted}
	start.Lease = lease(view.Action.LeaseGeneration+1, "executor:race", start.At)
	bindActionEvent(t, &start)
	clock = start.At
	if _, err := service.Transition(ctx, view.Action.ActionID, start); !errors.Is(err, ErrStoreConflict) {
		t.Fatalf("execution race error = %v, want ErrStoreConflict", err)
	}
	current, err := inner.Load(ctx, view.Action.ActionID)
	if err != nil {
		t.Fatal(err)
	}
	if current.State != actionlifecycle.StateAccepted || current.Revision != view.Action.Revision {
		t.Fatalf("Action changed after rejected responsibility race: %+v", current)
	}
}

func TestServiceRechecksAuthorizationAndLeaseAtCommit(t *testing.T) {
	t.Parallel()
	for _, test := range []struct {
		name       string
		commitAt   func(actionlifecycle.Snapshot, time.Time) time.Time
		mutateAuth func(*actionlifecycle.Event, time.Time)
		want       error
	}{
		{
			name:       "authorization expired",
			commitAt:   func(_ actionlifecycle.Snapshot, at time.Time) time.Time { return at.Add(6 * time.Second) },
			mutateAuth: func(event *actionlifecycle.Event, at time.Time) { event.Auth.ExpiresAt = at.Add(5 * time.Second) },
			want:       actionlifecycle.ErrInvalidEvent,
		},
		{
			name: "executor lease expired",
			commitAt: func(snapshot actionlifecycle.Snapshot, _ time.Time) time.Time {
				return snapshot.ExecutorLease.ExpiresAt
			},
			mutateAuth: func(event *actionlifecycle.Event, at time.Time) { event.Auth.ExpiresAt = at.Add(30 * time.Minute) },
			want:       actionlifecycle.ErrLeaseExpired,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			at := time.Date(2026, 8, 14, 20, 0, 0, 0, time.UTC)
			store, service, view, clock := newRunningActionService(t, at)
			complete := executorEvent(view.Action, actionlifecycle.EventComplete, at.Add(4*time.Second))
			complete.Reason = actionlifecycle.Reason{Code: actionlifecycle.ReasonCompleted}
			complete.ResultRef = "urn:result:delayed"
			test.mutateAuth(&complete, at)
			bindActionEvent(t, &complete)
			*clock = test.commitAt(view.Action, at)
			if _, err := service.Transition(context.Background(), view.Action.ActionID, complete); !errors.Is(err, test.want) {
				t.Fatalf("delayed commit error = %v, want %v", err, test.want)
			}
			stored, err := store.Load(context.Background(), view.Action.ActionID)
			if err != nil {
				t.Fatal(err)
			}
			if stored.State != actionlifecycle.StateRunning || stored.Revision != view.Action.Revision {
				t.Fatalf("expired mutation changed Action: %+v", stored)
			}
		})
	}
}

func TestServiceRejectsNewLeaseExpiredBeforeCommit(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	at := time.Date(2026, 8, 14, 20, 30, 0, 0, time.UTC)
	clock := at.Add(2 * time.Second)
	assignment := acceptedAssignment(t, "assignment:new-lease", "task:new-lease", "agent:owner", at)
	inner, err := NewMemoryStoreWithClock(
		[]taskcoord.Assignment{assignment}, nil, func() time.Time { return clock },
	)
	if err != nil {
		t.Fatal(err)
	}
	store := &executionDelayStore{MemoryStore: inner}
	service, err := NewService(store, func() time.Time { return clock })
	if err != nil {
		t.Fatal(err)
	}
	view, err := acceptWithTestAuth(t, service, ctx, assignment, clock, AcceptRequest{
		AssignmentID: assignment.AssignmentID, EventID: "event:accept:new-lease",
		ActionID: "action:new-lease", ActionDigest: "sha256:" + strings.Repeat("e", 64),
		RecoveryPolicy: actionlifecycle.RecoveryPolicy{
			Mode: actionlifecycle.RecoveryRestartIdempotent, MaxAttempts: 1,
			IdempotencyKey: "idempotency:new-lease",
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	startAt := at.Add(3 * time.Second)
	start := authenticatedActionEvent(view.Action, actionlifecycle.EventStart, startAt, "executor:new-lease")
	start.Reason = actionlifecycle.Reason{Code: actionlifecycle.ReasonStarted}
	start.Lease = lease(view.Action.LeaseGeneration+1, "executor:new-lease", startAt)
	start.Lease.ExpiresAt = startAt.Add(time.Second)
	bindActionEvent(t, &start)
	clock = startAt
	store.beforeCommit = func() { clock = start.Lease.ExpiresAt }
	if _, err := service.Transition(ctx, view.Action.ActionID, start); !errors.Is(err, actionlifecycle.ErrLeaseExpired) {
		t.Fatalf("expired new lease commit error = %v", err)
	}
	stored, err := inner.Load(ctx, view.Action.ActionID)
	if err != nil {
		t.Fatal(err)
	}
	if stored.State != actionlifecycle.StateAccepted || stored.Revision != view.Action.Revision {
		t.Fatalf("expired new lease changed Action: %+v", stored)
	}
}

func TestLeaseExpiryRequiresTrustedMonitor(t *testing.T) {
	t.Parallel()
	at := time.Date(2026, 8, 14, 21, 0, 0, 0, time.UTC)
	store, service, view, clock := newRunningActionService(t, at)
	expiry := view.Action.ExecutorLease.ExpiresAt
	untrusted := actionlifecycle.Event{
		ID: "event:untrusted-expiry", Kind: actionlifecycle.EventLeaseExpired,
		ExpectedRevision: view.Action.Revision, At: expiry,
		Reason: actionlifecycle.Reason{Code: actionlifecycle.ReasonLeaseExpired},
	}
	if _, err := service.Transition(context.Background(), view.Action.ActionID, untrusted); !errors.Is(err, ErrUseTrustedLeaseMonitor) {
		t.Fatalf("ordinary lease expiry error = %v, want %v", err, ErrUseTrustedLeaseMonitor)
	}

	*clock = expiry.Add(-time.Nanosecond)
	if _, err := service.ExpireLease(context.Background(), view.Action.ActionID); !errors.Is(err, actionlifecycle.ErrInvalidEvent) {
		t.Fatalf("early trusted expiry error = %v, want %v", err, actionlifecycle.ErrInvalidEvent)
	}
	current, err := store.Load(context.Background(), view.Action.ActionID)
	if err != nil || current.State != actionlifecycle.StateRunning {
		t.Fatalf("early expiry changed Action: %+v, %v", current, err)
	}

	*clock = expiry
	orphaned, err := service.ExpireLease(context.Background(), view.Action.ActionID)
	if err != nil {
		t.Fatal(err)
	}
	if orphaned.Action.State != actionlifecycle.StateOrphaned || orphaned.Action.ExecutorLease != nil {
		t.Fatalf("trusted expiry result = %+v", orphaned.Action)
	}
	if retry, err := service.ExpireLease(context.Background(), view.Action.ActionID); err != nil || retry.Action.Revision != orphaned.Action.Revision {
		t.Fatalf("trusted expiry retry = %+v, %v", retry.Action, err)
	}
}

type assignmentRaceStore struct {
	*MemoryStore
	once        sync.Once
	replacement taskcoord.Assignment
}

type executionDelayStore struct {
	*MemoryStore
	once         sync.Once
	beforeCommit func()
}

func newRunningActionService(
	t *testing.T,
	at time.Time,
) (*MemoryStore, *Service, View, *time.Time) {
	t.Helper()
	ctx := context.Background()
	clock := at.Add(2 * time.Second)
	assignment := acceptedAssignment(t, "assignment:deadline:"+at.Format("150405"), "task:deadline:"+at.Format("150405"), "agent:owner", at)
	...
	view, err := acceptWithTestAuth(t, service, ctx, assignment, clock, AcceptRequest{
		...
		EventID:      "event:accept:deadline:" + at.Format("150405"),
		ActionID:     "action:deadline:" + at.Format("150405"),
		ActionDigest: "sha256:" + strings.Repeat("f", 64),
		RecoveryPolicy: actionlifecycle.RecoveryPolicy{
			Mode: actionlifecycle.RecoveryRestartIdempotent, MaxAttempts: 1,
			IdempotencyKey: "idempotency:deadline:" + at.Format("150405"),
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	startAt := at.Add(3 * time.Second)
	start := authenticatedActionEvent(view.Action, actionlifecycle.EventStart, startAt, "executor:deadline")
	start.Reason = actionlifecycle.Reason{Code: actionlifecycle.ReasonStarted}
	start.Lease = lease(view.Action.LeaseGeneration+1, "executor:deadline", startAt)
	bindActionEvent(t, &start)
	clock = startAt
	view, err = service.Transition(ctx, view.Action.ActionID, start)
	if err != nil {
	...
	return store, service, view, &clock
}

func (s *assignmentRaceStore) CommitExecutionTransition(
	ctx context.Context,
	...
	s.once.Do(func() {
		...
		s.MemoryStore.assignments[s.replacement.AssignmentID] = cloneAssignment(s.replacement)
		s.MemoryStore.mu.Unlock()
	})
	return s.MemoryStore.CommitExecutionTransition(
		ctx, expectedAssignmentRevision, assignment, expectedActionRevision,
		current, event, transition, binding,
	)
}

func (s *executionDelayStore) CommitExecutionTransition(
	ctx context.Context,
	expectedAssignmentRevision uint64,
	assignment taskcoord.Assignment,
	expectedActionRevision uint64,
...
func acceptedAction(t *testing.T, actionID, ownerID string, at time.Time) actionlifecycle.Transition {
	...
	definition := actionlifecycle.Definition{
		...
		ActionDigest: actionDigest, OwnerID: ownerID,
		RecoveryPolicy: actionlifecycle.RecoveryPolicy{
			Mode: actionlifecycle.RecoveryRestartIdempotent, MaxAttempts: 2,
			IdempotencyKey: "idempotency:" + actionID,
		},
		AcceptanceContextDigest: "sha256:" + strings.Repeat("c", 64), AcceptedAt: at,
		Auth: &actionlifecycle.AuthenticatedOperation{
			ActorID: ownerID, AuthorizationID: "authorization:accept:" + actionID,
	...
	transition, err := actionlifecycle.NewSnapshot(definition)
	if err != nil {
...
func acceptWithTestAuth(
	...
	return service.Accept(ctx, request)
}

func bindTestAcceptance(
	t *testing.T,
	assignment taskcoord.Assignment,
	at time.Time,
	request *AcceptRequest,
	...
	request.Auth = &actionlifecycle.AuthenticatedOperation{
		ActorID: assignment.ParticipantID, AuthorizationID: "authorization:" + request.EventID,
		ProofID: "proof:" + request.EventID, Operation: actionlifecycle.EventAccept,
		ActionID: request.ActionID, ActionDigest: request.ActionDigest, MutationDigest: digest,
		VerifierNonce: "nonce:" + request.EventID, IssuedAt: at.Add(-time.Second), ExpiresAt: at.Add(15 * time.Minute),
	}
}
...
func authenticatedActionEvent(snapshot actionlifecycle.Snapshot, kind actionlifecycle.EventKind, at time.Time, actor string) actionlifecycle.Event {
	return actionlifecycle.Event{
		...
		Auth: &actionlifecycle.AuthenticatedOperation{
			...
			VerifierNonce: "nonce:" + actor + ":" + at.Format("150405"), IssuedAt: at.Add(-time.Second), ExpiresAt: at.Add(15 * time.Minute),
		},
	}
}
...
func bindActionEvent(t *testing.T, event *actionlifecycle.Event) {
	t.Helper()
...
func prepareDependencyWaitEvent(
	t *testing.T,
	binding Binding,
	current actionlifecycle.Snapshot,
	dependencies []taskcoord.Dependency,
