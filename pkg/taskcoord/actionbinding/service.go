...
// AcceptRequest contains caller-selected immutable Action fields and the
// verifier-created authorization for their exact acceptance. Owner and
...
func (s *Service) Accept(ctx context.Context, request AcceptRequest) (View, error) {
	...
	view, err := s.store.CommitAcceptance(ctx, assignment.Revision, assignment, request)
	if err != nil {
...
func (s *Service) Transition(ctx context.Context, actionID string, event actionlifecycle.Event) (View, error) {
	...
	if event.Kind == actionlifecycle.EventLeaseExpired {
		return View{}, ErrUseTrustedLeaseMonitor
	}
	if err := s.validateEventTime(event); err != nil {
		return View{}, err
	}
	executionTransition := startsOrExtendsExecution(event.Kind)
	if view.Assignment.Status != taskcoord.AssignmentAccepted && executionTransition {
	...
	if executionTransition {
		err = s.store.CommitExecutionTransition(
			ctx, view.Assignment.Revision, view.Assignment, view.Action.Revision,
			view.Action, event, transition, view.Binding,
		)
	} else {
		err = s.store.CommitAuthorizedTransition(ctx, view.Action.Revision, view.Action, event, transition)
	}
	if err != nil {
...
func (s *Service) WaitForDependencies(ctx context.Context, actionID string, event actionlifecycle.Event) (View, DependencyWait, error) {
	...
	if err := s.store.CommitDependencyWait(
		...
		dependencies, event, transition, view.Binding, wait,
	); err != nil {
...
// ExpireLease is the trusted lease-monitor path. It derives the observation
// timestamp and stable event identifier internally and the Store rechecks the
// stored lease with its own transaction clock before committing ORPHANED.
func (s *Service) ExpireLease(ctx context.Context, actionID string) (View, error) {
	view, err := s.Load(ctx, actionID)
	if err != nil {
		return View{}, err
	}
	if view.Action.State == actionlifecycle.StateOrphaned &&
		view.Action.LastTransition.Kind == actionlifecycle.EventLeaseExpired {
		return view, nil
	}
	if !view.Action.State.RequiresExecutorLease() || view.Action.ExecutorLease == nil {
		return View{}, actionlifecycle.ErrLeaseRequired
	}
	now := s.now().UTC()
	event := actionlifecycle.Event{
		ID:               leaseExpiryEventID(view.Action),
		Kind:             actionlifecycle.EventLeaseExpired,
		ExpectedRevision: view.Action.Revision,
		At:               now,
	...
	if err := s.store.CommitTrustedLeaseExpiry(ctx, view.Action.Revision, view.Action, event, transition); err != nil {
		return View{}, fmt.Errorf("commit trusted lease expiry: %w", err)
	}
	view.Action = transition.Snapshot
	return view, nil
}

func (s *Service) validateEventTime(event actionlifecycle.Event) error {
	now := s.now().UTC()
	if event.At.IsZero() || event.At.After(now) {
		return fmt.Errorf("%w: event time is not a trusted past or current timestamp", actionlifecycle.ErrInvalidEvent)
	}
	return nil
}

func leaseExpiryEventID(snapshot actionlifecycle.Snapshot) string {
	lease := snapshot.ExecutorLease
	raw := fmt.Sprintf("%s\x00%s\x00%d", snapshot.ActionID, lease.LeaseID, lease.Generation)
	digest := sha256.Sum256([]byte(raw))
	return "event:lease-expired:" + hex.EncodeToString(digest[:])
}
