...
func WaitForDependencies(
	...
	expectedCondition := actionlifecycle.ResumeCondition{Type: actionlifecycle.ResumeSignal, Signal: signal}
	if event.ResumeCondition == nil || *event.ResumeCondition != expectedCondition {
		return actionlifecycle.Transition{}, DependencyWait{}, fmt.Errorf("%w: event does not bind the exact dependency signal", ErrInvalidDependencyWait)
	}
...
func ResumeDependencyWait(
	...
	if event.EvidenceRef != evidence {
		return actionlifecycle.Transition{}, fmt.Errorf("%w: event does not bind the exact dependency evidence", ErrInvalidDependencyWait)
	}
	return actionlifecycle.Apply(current, event)
}

// DependencyResumeEvidence returns the exact evidence reference that a RESUME
// authorization must bind after the stored dependency topology is satisfied.
func DependencyResumeEvidence(
	binding Binding,
	assignment taskcoord.Assignment,
	current actionlifecycle.Snapshot,
	wait DependencyWait,
	...
	return dependencyResumeEvidence(binding, assignment, current, wait, dependencies)
}
